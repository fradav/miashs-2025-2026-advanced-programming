...
m_input = {
  const input = Inputs.range([64, 8192], {value: mutable m_value, step: 16, label: 'M (rows of A/C)'})
...
```{ojs}
k_input = {
  const input = Inputs.range([64, 8192], {value: mutable k_value, step: 16, label: 'K (cols of A / rows of B)'})
...
```{ojs}
n_input = {
  const input = Inputs.range([64, 8192], {value: mutable n_value, step: 16, label: 'N (cols of B/C)'})
...
```{ojs}
// Extract values from mutable variables only when run is triggered
dims = ({
...
```{ojs}
// Extract kernel and verify values only when run is triggered
kernel = mutable kernel_value
```

```{ojs}
verify = mutable verify_value
```
...
results = {
  ...
  try {
    ...
    if (!result) {
      return { error: 'runMatmul returned null/undefined' }
    }
