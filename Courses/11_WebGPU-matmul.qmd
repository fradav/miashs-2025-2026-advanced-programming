...
results_plots = {
  ...
  // Plotly layout
  const layout = {
    title: 'WebGPU Matrix Multiplication Performance',
    xaxis: {
      type: 'log',
      title: 'Memory (MB)',
  ...
  const legendsContainer = html`<div style="display:flex; flex-direction:column; gap:1rem; margin-bottom:1rem;"></div>`
  ...
  legendsContainer.append(metricsLegend)
  
  const fullContainer = html`<div class="results-plots"></div>`
  fullContainer.style.display = 'flex'
  fullContainer.style.flexDirection = 'column'
  fullContainer.style.gap = '1rem'
  fullContainer.append(legendsContainer)
